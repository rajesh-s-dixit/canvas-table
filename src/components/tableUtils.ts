...
export const createColumns = ({columns, theme: themeName}: ITableConfig, context: CanvasRenderingContext2D, height: number) => {4
  const theme = getTheme(themeName);
  let xOffset = 0;
...
export const getCell = (config: ITableConfig, event: React.MouseEvent<HTMLCanvasElement>) => {
